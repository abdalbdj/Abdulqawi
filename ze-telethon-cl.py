...
#logo
logo = pyfiglet.figlet_format('*      عبدالقوي      *')
print(X+logo)
print('  ')
print(A+'═'*60)
print('  ')
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        buttons = [
            [Button.inline('<سورس عبد القوي @shop_karel>', 'ze')],
...
async def handler(event):
    data = event.data.decode('utf-8')
    if data == 'ze':
        await bot.send_message(OWNER_ID, f"""**∘ بوت عبد القوي لتجميع النقاط واوامر اخرى 

∘ مطور ومبرمج البوت عبدالقوي - @shop_karel

∘ عبد القوي البوت = عبد القوي**""")
...
async def start(event):
    ...
    if sender.id == DEVELOPER_ID:
        ...
        [Button.inline('رشق مشاهدات ⟐', 'akbo')],
         ...
         [Button.inline('༺ مدفوع 𝐁𝐎𝐓 ༻', 'button0')]
        ]
